...
def enterprise_network():
    ...
    v1 = hm.Vulnerability("CVE-2015-3566",'port:80', values={'risk': 4.3, 'cost': 5.7, 'probability': 0.43, 'exploitability': 0.55,'impact': 5.5, 'defense_cost': 15})
    v2 = hm.Vulnerability("CVE-2000-1247",'port:1080', values={'risk': 2.1, 'cost': 7.9, 'probability': 0.21, 'exploitability': 0.29,'impact': 2.9, 'defense_cost': 15})
    v3 = hm.Vulnerability("CVE-2013-0638",'port:6660',values={'risk': 10.0, 'cost': 1.0, 'probability': 0.43, 'exploitability': 0.55, 'impact': 5.5, 'defense_cost': 15})
    v4 = hm.Vulnerability("CVE-2013-0900",'port:53',values={'risk': 4.3, 'cost': 7.9, 'probability': 0.23, 'exploitability': 0.29,'impact': 2.9, 'defense_cost': 15})
    v5 = hm.Vulnerability("CVE-2012-1675",'port:8080',values={'risk': 7.5, 'cost': 3.7, 'probability': 0.43, 'exploitability': 0.55,'impact': 5.5, 'defense_cost': 18})
    v6 = hm.Vulnerability("CVE-2016-2834",'',values={'risk': 8.8, 'cost': 1.2, 'probability': 1.0, 'exploitability': 0.64,'impact': 6.4, 'defense_cost': 18})
    v7 = hm.Vulnerability("CVE-2016-2834",'port:8080',values={'risk': 8.8, 'cost': 1.2, 'probability': 0.43, 'exploitability': 0.64,'impact': 6.4, 'defense_cost': 18})
    ...
    h1.lower_layer.basic_at([v1])
    h2.lower_layer.basic_at([v2])
    h3.lower_layer.basic_at([v3])
    h4.lower_layer.basic_at([v4])
    h5.lower_layer.basic_at([v5])
    ...
    enterprise[0].add_edge_between(h5, h6)
...
def simulation():
    ...
    for host in harm.top_layer.nodes():
        if host.name == "Attacker":
            Source = host
        if host.name == "206.17.1.47.7":
            Target = host
    harm[0].source = Source
    harm[0].target = Target
    harm[0].find_paths()
    harm.flowup()
    #hm.write_to_file(hm.convert_to_xml(harm), "C:\Python stuff\h\safeview\data\ADD_project\Net_vis.xml")
